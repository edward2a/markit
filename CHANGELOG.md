...
Versions are ordered newest-first: new change lines append inside the top section, new version sections are inserted directly below the `---` separator.
...
- 2026-09-23T01:35:20Z - mod - Defer the literal-search fast path and search index after final worker measurements showed about 450 us for a large-fixture scan and 3.5 ms for complete query churn; current background scans do not justify additional search memory or complexity
...
- 2026-09-21T23:57:04Z - mod - Build wrapper + strict C++17: new repo-root Makefile (help default, build, test, clean with BUILD_DIR/JOBS overridables; .gitignore keeps ignoring CMake-generated Makefiles via !/Makefile negation) and CMAKE_CXX_EXTENSIONS OFF so the build uses strict -std=c++17 instead of -std=gnu++17 on toolchains where C++17 is not default; clean rebuild, 194/194 + ctest + search/highlight/keybindings pty checks green
- 2026-09-22T01:53:33Z - add - Selectable UI background: new theme.background (default none = terminal background; named/hex colors), applied whole-window at the root so content measurement and search extraction are unaffected; ParseColor accepts none, --dump-config emits background: none; config unit tests + new check_background.py pty, 200/200 + ctest + all pty checks green

## 0.0.6 - 2026-09-09T17:15:43Z
- 2026-09-09T17:15:43Z - mod - Nav toggle moved from n to Ctrl+N (frees n/N for search navigation); action-bar hints updated in both foci, nav-highlight pty check drives Ctrl+N
- 2026-09-09T17:15:43Z - add - In-document search with RE2: / opens a prompt row (incremental count in the status bar), Enter accepts and jumps (n/N keep navigating the retained query with wrap-around, Esc cancels without jumping); invalid patterns report without matching; matches come from offscreen plain-text rows of the live tree (wide extraction in scroll mode) cached per tree/width/query/case; search_case_sensitive: false config; RE2 (2025-11-05) plus required Abseil (20250814.1) via FetchContent, linked PRIVATE; Matcher interface isolates the engine; 159/159 + ctest, check_search.py ALL OK in wrap and scroll
- 2026-09-09T17:48:19Z - mod - Fix multi-second input lag with search open: scroll-mode row extraction rendered offscreen at a hardcoded 8192 width over the full content height on every keystroke (hundreds of MB, blocking the event loop so even Esc/Ctrl+C stalled); extract at the tree's natural width via anchor.SearchExtractWidth and split the cache so typing is only a regex re-scan (width inputs cached, idle frames stay at pointer/int compares)
- 2026-09-09T20:15:45Z - mod - Search prompt lives in the action-bar row (/ query + inline Enter/Esc hints) instead of a dedicated row, so opening/closing never resizes the viewport or re-lays-out content
- 2026-09-09T20:15:45Z - mod - Search row extraction runs on one short-lived worker thread (private tree from immutable source, generation-guarded handoff, try_lock-only adoption, PostEvent wakeup, join-on-exit) with ... pending state: no input hitch at open or first keystroke; Helgrind reports 0 errors across search churn, mode toggles, and quit-mid-flight
- 2026-09-09T20:58:40Z - add - Valgrind helper script helper_scripts/vg_markit.sh: wrapper so the pty harness (which execs a single binary path) can drive markit under any valgrind tool (vg_markit.sh <tool> <args...>), run from the repo root
- 2026-09-10T01:29:34Z - add - Search highlight: the current match inverts in the live view while typing and while navigating the retained query with n/N (Esc/empty/invalid clears); spans come from a capture-capable RE2 pass over the single target row (Matcher::FindSpans, empty matches skipped), applied by a post-render SearchHighlight overlay node in content coordinates (same layout the rows are extracted with, so wrap/scroll/code/tables align; byte-to-cell walk handles wide characters); match state refresh split so the content renderer updates before drawing (no stale frame); 175/175 + ctest, check_search_highlight.py ALL OK both modes, Helgrind 0 errors over highlight churn, keystroke latency unchanged (~85ms)
- 2026-09-10T01:36:17Z - add - Spacebar pages down like PgDn: the scroller treats Space as PageDown in the main view, and the nav-focus branch moves the outline cursor a page on Space (skipped while the search prompt is open so it still types a space); unit test SpacePagesDown, 176/176 green
- 2026-09-10T08:24:27Z - add - Configurable keybindings: new top-level keybindings: section (24 actions, list-of-strings values) wired through the scroller and the main event chain, current mappings preserved as defaults and emitted by --dump-config; key names are specials (up/down/left/right/pageup/pagedown/home/end/tab/enter+return/esc+escape/space/slash+/) case-insensitively, case-sensitive single chars, ctrl+<letter>; specifying an action replaces its list (empty unbinds), unknown actions/keys and same-context duplicates (content/nav-focus/search-prompt groups; modal overlaps like Esc cancel/quit and Enter accept/activate stay allowed) fail fast with dotted-path errors; 192/192 + ctest, new pty helper_scripts/check_keybindings.py ALL OK, existing search/nav/highlight pty checks re-green
...
- 2026-09-09T11:48:38Z - add - Interactive nav bar: Tab switches focus between main view and outline (only focus switch; Esc still quits), arrows/k/j/Home/End/PgUp/PgDn move a keyboard cursor rendered inverted, Enter jumps the main view to the section (focus stays in nav), Outline title inverts while focused; cursor syncs to the section in view on Tab-in, follow-scrolls long outlines via ClampNavOffset/FollowNavOffset, and combines with the bold-yellow view highlight; NavBar takes optional cursor/focused, ActionBar shows nav keys when focused
