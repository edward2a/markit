...
def drive_to(ses, name, max_presses, show, tag):
    ...
    for _ in range(max_presses):
        raw, ch, fg, bg, bo, _ = ses.frame(keys=b"j")
...
def main():
    ...
    try:
        ...
        raw, ch, fg, bg, bo, _ = ses.frame(keys=b"\x1b[H")
        if show:
            print("--- frame home ---")
            for r in range(ROWS):
                print("%2d|%s" % (r, "".join(ch[r]).rstrip()))
        ...
        raw, ch, fg, bg, bo, _ = ses.frame(keys=b"\x1b[F")  # End
        rg = only_highlighted(ch, fg, bo, "Gamma")
        check("Gamma highlighted at End", rg is not None,
              "navrow=%s" % rg)

        raw, ch, _, _, _, _ = ses.frame(keys=b"\x0e")  # Ctrl+N hides
        check("nav hidden",
              find_row(ch, "Gamma", NAV_COL) < 0)
        raw, ch, fg, bg, bo, _ = ses.frame(keys=b"\x0e")  # Ctrl+N restores
        rg = only_highlighted(ch, fg, bo, "Gamma")
